...
<script type="text/javascript">
    $(function () {
        $("#btn-register").on("click",function () {
            ...
            $.post("${pageContext.request.contextPath}/teacher/register",{account:account,password:password},registerSucceed,"json")
        })
        $("#btn-login").on("click",function () {
            const account=$("#account").val();
            const password=$("#Password").val();
            $.post("${pageContext.request.contextPath}/teacher/login",{account:account,password:password},loginSucceed,"json")
        })
    })
    ...
    function loginSucceed({flag, message}) {
        alert(message);
        if (flag){
            $(window).attr("location","teacherIndex.jsp")
        }
    }
</script>
<body>
